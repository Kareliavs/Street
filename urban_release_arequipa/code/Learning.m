    ...
    methods(Static)
        ...
        function data = CreateRandomizedSplit(images, labels, type)
            if strcmp(type, 'classification')
                ...
                data.test_images = [pos_images(num_train_pos + num_val_pos + 1 : end); ...
                                    neg_images(num_train_neg + num_val_neg + 1 : end)];
        ...
        function test = TestLinearSvm(data, features_set, model)%importante%------------------------!!!!!!!!!!!!!!!!!!!
            ...
            [predicted_labels, accuracy, decision_values] = ... % valor de prediccion ---------------------------------------
...
%                test_labels = 10 ./ (1 + exp(-(data.test_labels - mean(data.test_labels))));
        ...
        function model = TrainLinearSvm(data, features_set, config)%MAS!!! importante%------------------------!!!!!!!!!!!!!!!!!!!
            if ~isfield(config, 'svm_method') || isempty(config.svm_method)
                config.svm_method = Learning.SVM_TYPE('L2_REGULARIZED_L2_LOSS_DUAL'); %L2 regularizado
            end
            if ~isfield(config, 'bias_term') || isempty(config.bias_term)
                config.bias_term = 1;
            end
            ...
            for i = 1 : length(c_param_space) 
                ...
                model_ = train(data.train_labels, sparse(features.train_features), ...
                               sprintf('-s %d -B %d -c %f -q -w-1 1 -w1 %f', ...
                                       config.svm_method, config.bias_term, c, wneg));
                % Test on validation set and compute accuracy.
                [predicted_labels, accuracy, decision_values] = ...
                    predict(data.val_labels, sparse(features.val_features), model_, '-q'); 
                accuracy = accuracy(1);
        ...
        function model = TrainKernelSvm(data, features, config)
            ...
            % Test on trainval set and compute accuracy.
            [predicted_labels, accuracy, decision_values] = ...
                svmpredict(trainval_labels, sparse(trainval_features), libsvm_model, '-q -b 1'); %libreria --> /home/karelia/Documentos/2018/Street/urban_release/lib/liblinear-1.94/matlabpredict.c
            accuracy = accuracy(1);
