...
classdef UrbanPerception
    ...
    methods
        function obj = UrbanPerception(data_file, data_format)
            obj.data = UrbanPerception.LoadData(data_file, data_format);
        end
        ...
        function plotData(obj, city_str, metric_str, output_str, image_url)
            ...
            for ii = 1 : n_pages
                ...
                for jj = 1 + (ii - 1) * results_per_page : min(length(scores), ii * results_per_page)
                    ...
                    fprintf(f, '<td><img src="%s/%s" height="180"/></td>', ...
                            image_url, data.image_names{sorted_inds(jj)});
